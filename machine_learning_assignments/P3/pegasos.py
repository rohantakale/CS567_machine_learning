...
def objective_function(X, y, w, lamb):
    ...
    N = float(X.shape[0])
    regln = 0.5 * lamb * w_norm * w_norm 
    loss = np.maximum(0.0, 1.0 - elem_mult(np.matmul(X,w), y))
    obj_value = regln + np.sum(loss)/N
...
def pegasos_train(Xtrain, ytrain, w, lamb, k, max_iterations):
    ...
    for iter in range(1, max_iterations + 1):
        ...
        w0 = (1.0 - nt * lamb) * w
        for i in A_t:
            value = ytrain[i] * np.dot(Xtrain[i], w)
            if value < 1:
                y_x = (ytrain[i] * Xtrain[i]).reshape(-1, 1)
                w0 = w0 + (nt / float(k)) * y_x
        w0_norm = np.linalg.norm(w0)
        if w0_norm != 0:
            w_val = min(1.0, (1.0 / np.sqrt(lamb)) / w0_norm)
            w = w_val * w0
        else:
            w = w0
...
def pegasos_test(Xtest, ytest, w_l):
    ...
    count = 0
    pred = np.matmul(Xtest, w_l)
    pred_signs = np.sign(pred)
    for i in range(N):
        if pred_signs[i] == ytest[i]:
            count += 1
    test_acc = float(count / N)
