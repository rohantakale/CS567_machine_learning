...
def pca(X = np.array([]), no_dims = 50):
    ...
    covar_matx = np.matmul(X.T, X)

    # eigenvalues in ascending order 
    eig_value, eig_vector = np.linalg.eigh(covar_matx)
    eig_value = np.flip(eig_value, axis=0) 
    eig_vector = np.flip(eig_vector, axis=1)

    # normalize eigen vectors
    for j in range(0, no_dims):
        eig_vector[:, j] /= np.linalg.norm(eig_vector[:, j])
    M = eig_vector[:, :no_dims]
    Y = np.matmul(X, M)
